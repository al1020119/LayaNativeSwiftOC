...
+(void)hideSplash {
    ...
    [[UIViewViewController GetIOSViewController].launchView hide];
}

+(void)setTips:(NSArray*)tips {
    AppDelegate * appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
    [UIViewViewController GetIOSViewController].launchView.tips = tips;
}

+(void)setFontColor:(NSString*)color {
    AppDelegate * appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
    [[UIViewViewController GetIOSViewController].launchView setFontColor:color];
}

+(void)bgColor:(NSString*)color {
    AppDelegate * appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
    [[UIViewViewController GetIOSViewController].launchView setBackgroundColor:color];
}

+(void)loading:(NSNumber*)percent {
    AppDelegate * appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
    [[UIViewViewController GetIOSViewController].launchView setPercent:percent.integerValue];
}

+(void)showTextInfo:(NSNumber*)show {
    AppDelegate * appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
    [[UIViewViewController GetIOSViewController].launchView showTextInfo:show.intValue > 0];
}
...
+ (void)getReadInfo {
    ...
    NSDictionary *dictM = @{@"novelid": @"5399"};
